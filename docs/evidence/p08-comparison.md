...
The checked, previewable page is
[intent-composer.md](../../features/evidence/client/src/intent-composer/intent-composer.md)
(state diagram, then the effect-machine Confirm sequence).
...
A second hand-drawn copy of the state diagram would be a **cache**. It can
drift from the driver. **Visibility of the legal moves is part of
IntentComposer**: concept page, lesson, and exemplar show one graph that is
generated from the machine and checked in `verify`. If the machine changes
and the diagram does not, verify fails.

**Now:** [`intent-composer.md`](../../features/evidence/client/src/intent-composer/intent-composer.md)
is generated by `intentComposerTeachingDoc` in
[`graph.ts`](../../features/evidence/client/src/intent-composer/graph.ts)
from `intentComposerGraph`. The state diagram is first; the Confirm sequence
for effect-machine follows. The effect-machine `handle` reads every target
from that same graph. A unit test asserts the generator output equals the
committed page, so `verify` fails when they drift. Open the markdown to see
the diagrams; a bare `.mmd` file does not preview.
